...
// `eth_getLogs` and has to answer the same question when a page fails: was the *query* too big, or is this
// caller asking too often? The two failures need opposite responses. A range cap is fixed by halving the
// window; a rate limit is not, because a narrower window is still one more request per second, and halving
// against a throttled node just burns the halving budget and reports a raw provider string. BSC's
// `data-seed-prebsc-*` endpoints answer every `eth_getLogs` with JSON-RPC -32005 "limit exceeded" whatever
// the range, which is exactly the case halving cannot win.
//
 ...
 */
export type ProviderErrorKind = "rangeCap" | "rateLimit" | "unknown";
...
/**
 * Splits a provider refusal into the three kinds above (SPEC §10.1).
 *
 * The range wordings are tested first on purpose: "limit exceeded" is a bare rate limit on BSC's data seeds,
 * but the same substring also appears inside longer range-cap sentences, and answering a range cap with a
 * wait would stall a scan that a halving would have finished.
 */
